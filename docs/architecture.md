...
- **No service layer for plain CRUD** — `Pokedex.API` controllers (`BasePokemonController`, `MovesController`, `MyPokemonController`, Part 2) call the relevant repository directly (e.g., `IMyPokemonRepository` + `ISaveChanges`), with no intermediate service class. There is no orchestration, no multi-aggregate coordination, and no business rule beyond simple validation (e.g., max 4 moves) for these endpoints — wrapping them in a service would just forward calls 1:1 to the repository, adding a layer with no behavior of its own. This is deliberately asymmetric with `Battles.API` (see Battle Turn Orchestrator below), which *does* need a service because it coordinates multiple aggregates atomically.
...
- **Implementations live in `Pokemons.Infra`**: EF Core-backed repository classes + `PokemonsDbContext`. The `DbContext` itself is never exposed outside `Pokemons.Infra`.
